...
let () =
  ...
  flush stdout ;
  chunk_files :=
    Io.File_utils.read_chunks ~filename:output_txt ~chunk_size_mb
      ~f:(fun chunk_text ->
        flush stdout ;
  ...
  flush stdout ;
  Wordvec.SkipGram.train model pairs ~neg_table ~neg_samples ~epochs ;
  ...
  Wordvec.SkipGram.save ~filename:"data/artifacts/embeddings.csv" ~model
    ~embed_dim ~sorted_vocab ~vocab_tbl ;
