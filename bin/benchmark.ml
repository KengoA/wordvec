...
let parse_wordsim353 filename =
  ...
  let rec loop acc =
    ...
    | line -> (
      ...
      match cols with
      | w1 :: w2 :: score :: _ -> (
        try loop ((w1, w2, float_of_string score) :: acc)
        with Failure _ ->
...
let evaluate_wordsim353 embeddings_file wordsim_file =
  ...
  let cosines, scores =
    List.fold_left
      (fun (coss, scs) (w1, w2, score) ->
        match
          ( Array.fold_left
              (fun acc (i, w) -> if w = w1 then Some i else acc)
              None
              (Array.mapi (fun i w -> (i, w)) words),
            Array.fold_left
              (fun acc (i, w) -> if w = w2 then Some i else acc)
              None
              (Array.mapi (fun i w -> (i, w)) words) )
        with
        | Some idx1, Some idx2 ->
          let cos =
            Wordvec.Core.Similarity.cosine_similarity embeddings.(idx1)
              embeddings.(idx2)
          in
          (cos :: coss, score :: scs)
        | _ -> (coss, scs))
...
let () =
  evaluate_wordsim353 "data/artifacts/embeddings.csv"
    "data/eval/wordsim353crowd.csv"
