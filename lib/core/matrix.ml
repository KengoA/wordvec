...
module Matrix : MatrixSig = struct
  ...
  let random rows cols = Array.init rows (fun _ -> Array.init cols (fun _ -> Random.float 1.0))
  ...
  let dot a b = 
    ...
    res
  let add a b = Array.mapi (fun i row -> Array.mapi (fun j v -> v +. b.(i).(j)) row) a
  let sub a b = Array.mapi (fun i row -> Array.mapi (fun j v -> v -. b.(i).(j)) row) a
