let cosine_similarity v1 v2 =
  ...
  if !norm1 = 0. || !norm2 = 0. then 0. else !dot /. (sqrt !norm1 *. sqrt !norm2)
...
let pearson_correlation xs ys =
  ...
  in
  let sx =
    sqrt (List.fold_left (fun acc x -> acc +. ((x -. mx) ** 2.)) 0. xs)
  in
  let sy =
    sqrt (List.fold_left (fun acc y -> acc +. ((y -. my) ** 2.)) 0. ys)
  in
...
let knn vectors idx k =
  ...
  let sims =
    Array.mapi
      (fun j wv -> if j = idx then (-1.0, j) else (cosine_similarity v wv, j))
      vectors
  in
