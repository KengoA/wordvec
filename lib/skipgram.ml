...
module type SkipGramSig = sig
  ...
  val train : t -> (int * int) Seq.t -> neg_table:int array -> neg_samples:int -> epochs:int -> unit
  val get_embedding : t -> int -> float array
  val save: filename:string -> model:t -> embed_dim:int -> sorted_vocab:(string * 'a) list -> vocab_tbl:(string, int) Hashtbl.t -> unit
...
module SkipGram : SkipGramSig = struct
  ...
  let create ~vocab_size ~embed_dim = {
    w_in = Array.init vocab_size (fun _ -> Array.init embed_dim (fun _ -> Random.float 1.0));
    w_out = Array.init vocab_size (fun _ -> Array.init embed_dim (fun _ -> Random.float 1.0));
    embed_dim;
  }
  ...
  let sgd_step model input_idx context_idx neg_table neg_samples lr =
    ...
    let update output_idx label =
      ...
      for i = 0 to model.embed_dim - 1 do
        ...
        model.w_in.(input_idx).(i) <- model.w_in.(input_idx).(i) -. lr *. grad_in;
        model.w_out.(output_idx).(i) <- model.w_out.(output_idx).(i) -. lr *. grad_out;
      done
    in
  ...
  let train model pairs ~neg_table ~neg_samples ~epochs =
    ...
    for epoch = 1 to epochs do
      Seq.iter (fun (input_idx, context_idx) ->
  ...
  let get_embedding model idx =
    Array.copy model.w_in.(idx)
    ...
    (try
      while true do
        ...
        | word :: floats when List.length floats = dims ->
          let vec = Array.init dims (fun i -> float_of_string (List.nth floats i)) in
          words := word :: !words;
          vectors := vec :: !vectors
        | _ -> ()
      done
